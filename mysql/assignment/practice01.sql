...
select distinct count(emp_no) as '직원 수' from salaries;
...
select distinct count(dept_no) as '부서 매니저 수' from dept_manager;
...
/** 문제 10: 현재 Enginneer 직책의 사원은 총 몇 명입니까? */
select count(*)
  from titles
 where title = 'Engineer' and to_date > sysdate();
